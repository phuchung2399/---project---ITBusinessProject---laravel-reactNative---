...
class CommentSerivece
{
    ...
    {
        try {
            if ($this->commentRepository->checkCommentById($id) && $this->storeRepository->checkStoreById($request->store_id)) {
                $comment = $this->commentRepository->selectCommentById($id);
                if (Auth::user()->user_id == $comment->user_id && $comment->store_id == $request->store_id) {
                    $this->handleRequest($comment, $request);
                    ...
                    return Response::responseMessage(HttpStatus::BAD_REQUEST, "Bạn không có quyền sửa bình luận này");
                }
            } else {
