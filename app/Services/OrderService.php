...
class OrderService
{
    ...
    function insertOrder($request)
    {
        try {
            ...
            $order->address = Validation::handleSpace($request->address);
            ...
            $order->store_id = ($request->store)["store_id"]; // $request->store_id;
        ...
        try {
            if ($this->orderRepository->checkOrderById($id)) {
                ...
                if ($order->massage_id == 16) {
                    ...
                    return Response::responseMessage(HttpStatus::BAD_REQUEST, 'Đơn đã hoàn thành nên không thể hủy');
                }
            }
        } catch (\Exception $exception) {
            return Response::responseMessage(HttpStatus::BAD_REQUEST, $exception->getMessage());
